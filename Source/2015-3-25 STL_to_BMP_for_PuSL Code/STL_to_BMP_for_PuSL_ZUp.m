...
YUp = 0; %If YUp = 1 Y is the up orientation, if YUp = 0 Z is the up orientation.
%If YUp = 2 x is the up orientation.
%Designs from Thingiverse.com have Z-Up.
%Most designs originally for our PuSL printer have YUp.
...
fname='Madeleine.STL'; %Filename of Dec 27 file
...
%Outputs:
dir_bmap='Madeleine2';    %Output folder for saving bitmap files
%ofname_prefix='springy';
mkdir(dir_bmap);
ofname_prefix=dir_bmap;   %Prefix of output file for bitmaps

nn=round(7/0.020);    %Height of spring structure/20microns for old file
%round(3.4/0.020) = 170 layers
%nn=180;    %Height of spring structure/20microns for new file

%Topology Optimized Designs are 4mmx4mm
wid=round(7/0.0071);   %Width of spring structure/7.1microns (this is X, 1050 pixel limit))
len=round(7/0.0071);   %Length of spring structure/7.1microns (this is Y, 1400 pixel limit)
...
spacer = 60;
Exp=30; %Exposure Time was 12 seconds
...
if YUp ==0;
    [OUTPUTgrid] = VOXELISE(wid,len,nn,fname,'xyz');
end
if YUp ==2;
    [OUTPUTgrid] = VOXELISE(nn,len,wid,fname,'xyz');
end
...
if YUp ==0;
    [OUTPUTgrid2] = VOXELISE(wid,len,nn,fname2,'xyz');
end
if YUp ==2;
    [OUTPUTgrid2] = VOXELISE(nn,len,wid,fname2,'xyz');
end   
%The final structure will drop these springs onto a volume of dimension:
% x:1400, y:1050, z=height of structure/20microns = 3.4mm/20microns=170
