...
% 1. Designate the number of input files (1 or 2)
% 2. Designate how many structures you want to print (1, 2 or 4 for a
% single file, 1 or 2 for 2 files).
% 3. Make sure your file is in the current working directory.
% 4. Copy your first filename under fname
% 5. (optional) copy 2nd filename under fname2
% 6. Choose your 'output name' under dir_bmap. This is the name of the
% folder and the name of each bitmap in the folder
% 7. Input Solidworks Y distance for the variable nn. Round(YY/0.020).
% 0.020 represents the thickness of each layer
% 8. Input Solidworks X distance for the variable wid. Round(XX/0.0071).
% 0.020 represents the projected pixel size
% 9. Input Solidworks Z distance for the variable len. Round(ZZ/0.0071).
% 0.0071 represents the projected pixel size
...
fname= 'HW350umStentRevised3.STL'; %Filename of Dec 27 file

if numinputfiles == 2;
fname2='IntraOcular Lens Y Up.STL'; %The second input file name
%NOTE: both structures should have same dimensions
end

%Outputs:
dir_bmap='350umStentRevised3';    %Output folder for saving bitmap files
%ofname_prefix='springy';
mkdir(dir_bmap);
ofname_prefix=dir_bmap;   %Prefix of output file for bitmaps

nn=round(21.80/0.020);    %Height of spring structure/20microns for old file
%round(3.4/0.020) = 170 layers
%nn=180;    %Height of spring structure/20microns for new file

%Topology Optimized Designs are 4mmx4mm
wid=round(5.5/0.0071);   %Width of spring structure/7.1microns
len=round(5.5/0.0071);   %Length of spring structure/7.1microns
